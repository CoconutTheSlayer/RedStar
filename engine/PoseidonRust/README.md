...
- **The C header is generated by cbindgen** (`build.rs` + `cbindgen.toml`) from
  `src/lib.rs` on every build, so it can never drift from the Rust signatures.
  `include/poseidon_rust.h` is committed (present for IDEs/consumers without a
